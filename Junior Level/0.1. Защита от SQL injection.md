...
2. Параметризованные запросы
   > Это, по сути, одно и то же с точки зрения защиты от SQL-инъекций. В большинстве языков реализовано как “prepared statements” (подготовленные выражения), иногда это называют “параметризованными запросами”.
3. Валидация и фильтрация пользовательских данных;
4. Минимальные права у SQL-пользователя;
5. SAST/DAST & Code Review;
6. Периодическое обучение разработки.
...
Подготовленные — всегда параметризованные, но не наоборот. В повседневном AppSec их не разделяют: важно, чтобы параметры не попадали в SQL как строки напрямую.
